...
Redlining Maps w/ Race and Per Capita Income  -- Shreya (Work in Progress right now)
```{r}
...
balt_race_2020 <- race_2020 %>%
  clean_names() %>%
  group_by(name) %>%
  pivot_wider(names_from=c("variable"), values_from=c("estimate", "moe")) %>%
  rename(
    total_population = estimate_B03002_001,
    white_non_hispanic = estimate_B03002_003,
    black_alone = estimate_B03002_004,
    asian = estimate_B03002_006,
    hispanic_latino = estimate_B03002_012
  ) %>% 
  separate(name, into=c("tract","name","state"), sep=",") %>%
  mutate(
      state = str_trim(state,side="both"),
...
rlining_and_med_income <- leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
...
rlining_and_med_income

saveWidget(rlining_and_med_income, "redlining_and_med_income.html")
