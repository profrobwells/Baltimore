...
Redlining Maps -- Shreya (Work in Progress right now)
```{r}
...
## weird that polygons aren't showing up even though shapefile is read in --> will take a look at this later. 
rlining_and_ag_income <- leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(
    data = balt_redlining
  ) 

rlining_and_ag_income
