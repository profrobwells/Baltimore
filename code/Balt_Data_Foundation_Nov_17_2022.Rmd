---
title: "Baltimore Data Reference"
author: "Rob Wells"
date: "11/17/2022"
...
# New Per Capita Income

```{r}
#2020 Per Capita By Place ACS
#2016-2020
...
percap_income2020 <- get_acs(geography = "place", 
...
```{r}
#2016 & 2010 Per Capita Income By Place ACS

percap_income2016 <- get_acs(geography = "place", 
              variables = c(number_households = "B19001_001", percap_income = "B19301_001"), 
               state = "MD", 
               year = 2016) %>% 
  mutate(year=("2016"))
...
percap_income2010 <- get_acs(geography = "place", 
              variables = c(number_households = "B19001_001", percap_income = "B19301_001"), 
               state = "MD", 
               year = 2010) %>% 
  mutate(year=("2010"))
...
percap_md_all <- separate(data = percap_md_all, col = NAME, into = c("Place", "County", "State"), sep = ",", extra = "merge", fill = "right")
...
baltimore_city <- percap_md_all %>% 
  filter(place=="Baltimore city") 
  
#write.csv(baltimore_city, "baltimore_city_percap.csv")
```
...
## call in percap income csv from folder
...
### 
baltcity_percap_income <- baltcity_percap_income %>%
  mutate(diff_2020_2010 = as.numeric(diff_2020_2010)) %>%
  st_as_sf(crs = wgs,
           coords = geometry)
...
rlining_and_demographics <- leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  ##this chunk below is just so we can search by neighborhood later
  addPolylines(
    group = "Neighborhood",
    data = baltcity_percap_income,
    color = "Gray",
    weight = 1,
    label = baltcity_percap_income$community_statistical_area_2020,
    fillOpacity = 0.75,
...
## NOTES 
## - switching polygons to polylines makes it so you can't hover
## - make seperate colornumeric with defined values? 
## - any updates on whether to include C vs D regions?
